...
use crate::{
    ...
    audio::sound_effect,
    game::{
        level::{Level, LevelEntryOverlay, bosses::Boss, enemies::Enemy, sfx_intro, spawn_level},
        player::*,
    },
    menus::Menu,
    screens::Screen,
...
/// "1 boss per level, if boss gets life zero, auto move on?" "send event to level transition"
fn check_boss_and_player(
    mut next_menu: ResMut<NextState<Menu>>,
    boss_q: Query<(Entity, &Enemy), With<Boss>>,
    player_q: Single<&Player>,
    trans_q: Query<Entity, With<LevelTransitionOverlay>>,
    mut cmd: Commands,
) {
    match boss_q.single() {
        Ok((boss_entity, boss_enemy)) => {
            if boss_enemy.life == 0 && trans_q.is_empty() {
...
fn despawn_finished_transitions(
    mut cmd: Commands,
    query: Query<
        (Entity, &LoadingFadeInOut),
        Or<(With<LevelEntryOverlay>, With<LevelTransitionOverlay>)>,
    >,
) {
